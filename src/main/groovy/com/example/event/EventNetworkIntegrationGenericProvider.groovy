...
@Slf4j
class EventNetworkIntegrationGenericProvider extends AbstractGenericIntegrationProvider implements GenericIntegrationProvider.EventSubscriberFacet<NetworkEvent> {

	public static final String PROVIDER_CODE = 'morpheus-event-sample.generic'
	public static final String PROVIDER_NAME = 'Event Network Integration Generic Provider'

	protected MorpheusContext context
	protected Plugin plugin

	EventNetworkIntegrationGenericProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
	...
	AccountIntegrationType.AssociationType getClusterAssociationType() {
		return AccountIntegrationType.AssociationType.ONE
	}
	...
	void onEvent(NetworkEvent networkEvent, AccountIntegration integration) {
		log.info("Network Event Detected! - ${integration.name} - ${networkEvent.dump()} - ${networkEvent.network?.name}")
	}
}
