...
@Slf4j
class EventNetworkManyClusterIntegrationGenericProvider extends AbstractGenericIntegrationProvider implements GenericIntegrationProvider.EventSubscriberFacet<NetworkEvent> {

	public static final String PROVIDER_CODE = 'morpheus-event-many-cluster-sample.generic'
	public static final String PROVIDER_NAME = 'Event Network Many Cluster Integration Generic Provider'

	protected MorpheusContext context
	protected Plugin plugin

	EventNetworkManyClusterIntegrationGenericProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
	...
	List<EventType> getSupportedEventTypes() {
		return [NetworkEvent.NetworkEventType.CREATE, NetworkEvent.NetworkEventType.UPDATE, NetworkEvent.NetworkEventType.DELETE]
	}

	@Override
	List<EventScope> getEventScopes() {
		return [EventScope.CLUSTER]
	}

	@Override
	void onEvent(NetworkEvent networkEvent, AccountIntegration integration) {
		log.info("Network Event Detected! - ${integration.name} - ${getEventScopes().join(",")} - ${networkEvent.dump()} - ${networkEvent.network?.name}")
	}
}
